...
<!--  -->
    <v-card class="d-flex flex-row justify-center" color="grey lighten-2" flat tile>
      <v-btn fab tile @click="clear()">AC</v-btn>
      <v-btn fab tile @click="del()">del</v-btn>
      <v-btn fab tile @click="addExpresion('%')">%</v-btn>
      <v-btn fab tile @click="addExpresion('/')">/</v-btn>
    </v-card>
<!--  -->
    <v-card class="d-flex flex-row justify-center" color="grey lighten-2" flat tile>
      <v-btn v-for="r2 in row2" :key="r2" fab tile @click="addExpresion(r2)">{{r2}}</v-btn>
    </v-card>
<!--  -->
    <v-card class="d-flex flex-row justify-center" color="grey lighten-2" flat tile>
      <v-btn v-for="r3 in row3" :key="r3" fab tile @click="addExpresion(r3)">{{r3}}</v-btn>
    </v-card>
<!--  -->
    <v-card class="d-flex flex-row justify-center" color="grey lighten-2" flat tile>
      <v-btn v-for="r4 in row4" :key="r4" fab tile @click="addExpresion(r4)">{{r4}}</v-btn>
    </v-card>
<!--  -->
    <v-row no-gutters style="flex-wrap: nowrap;">
      <v-col class="flex-shrink-1" style="min-width: 50%; max-width: 100%;">
        <v-btn class="pa-7" tile style="min-width: 100%; max-width: 100%;" @click="addExpresion(0)">0</v-btn>
      </v-col>
      <v-col class="flex-shrink-1" style="min-width: 25%; max-width: 100%;">
